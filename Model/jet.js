...
Jet.prototype.checkShooting = function () {
  if (this.jetWarOptions.fireBtn && shootInterval > 5) {
    ...
    this.jetWarOptions.bullets[this.jetWarOptions.currentBullet++].fire(
      this.options.drawX + 100,
      this.options.drawY + 30
    );
...
Jet.prototype.checkHitWall = function () {
  ...
  if (this.options.drawX + this.options.width >= window.innerWidth)
    this.options.drawX = window.innerWidth - this.options.width;
  if (this.options.drawX <= 0) this.options.drawX = 0;
  if (this.options.drawY <= 0) this.options.drawY = 0;
  if (this.options.drawY + this.options.height >= window.innerHeight - 250)
    this.options.drawY = window.innerHeight - 250 - this.options.height;
};
  ...
  for (var i = 0; i < canvas.enemies.length; i++) {
    if (detectCollision(this, canvas.enemies[i])) {
      ...
      if (this.life <= 0) {
        ...
        xhttp.open(
          "POST",
          "https://admin.crimsonapps.net/v1/data/achievement",
          true
        );
